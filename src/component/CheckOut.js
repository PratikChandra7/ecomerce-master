...
const CheckOut = ({ open, setOpen }) => {
  ...
  const onDeleteCartItem = (item) => {
    let temp = cartItem.filter((val) => val !== item[0].id);
    onSetCartCount(temp);
  };

  const { cartItem, onSetCartCount, login } = useContext(Context);
  const onPlaceOrder = () => {
      ...
      <Box sx={{ ...style, width: "50%" }}>
        ...
        {cartItem.length > 0 ? (
                ...
                >
                  <Tooltip title="delete">
                    <DeleteIcon
                      ...
                      onClick={onDeleteCartItem.bind(this, filter)}
                    />
                  </Tooltip>
                </div>
              </div>
          ...
          <p id="parent-modal-description">
            Sorry! No Product available in cart
          </p>
        )}
        {total > 0 && (
          <>
        ...
        {enableNextPage && cartItem.length > 0 && (
          <>
            {login ? (
              <>
                <Address
        ...
        {proceedAs === "guest" && visibleAddress && cartItem.length > 0 && (
        ...
        >
          ...
          <h5 style={{ padding: "20px" }}>Order Successfully Placed</h5>
